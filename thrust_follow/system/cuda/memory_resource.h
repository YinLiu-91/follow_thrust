...
{
public:
	Pointer do_allocate(std::atomic_size_t bytes, std::size_t alignment = THTRUST_MR_DEFAULT)THRUST_OVERRIDE
	{
		(void)alignment;
		...
		if (status != cudaSuccess) {
			...
			throw thrust::system::bad_alloc(thrust::cuda_category().message(status).c_str());
		}
		return Pointer(ret);
	}

	void do_deallocate(std::atomic_size_t bytes, std::size_t alignment)THRUST_OVERRIDE
	{
		(void)bytes;
		(void)alignment;
...
typedef detail::cuda_memory_resource<cudaMallocHost, cudaFreeHost,//固定（主机）内存
